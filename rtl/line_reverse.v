...
//
// line_reverse.v
// Line reversal buffer for EPD controller
...
module line_reverse #(
    ...
    output wire [PIXWIDTH-1:0] pix_out,
    ...
    // Memory read ports - completely isolated to allow block RAM inference
    reg [PIXWIDTH-1:0] buf0_out;
    reg [PIXWIDTH-1:0] buf1_out;

    always @(posedge clk) begin
        if (rd_buffer_valid) begin
            buf0_out <= buf0[rdptr_next];
            buf1_out <= buf1[rdptr_next];
        end
    end

    // Combinational multiplexer to output
    assign pix_out = (rd_buffer) ? buf1_out : buf0_out;
