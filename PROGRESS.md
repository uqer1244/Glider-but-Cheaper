...
가로 2배 확대 배선, 클럭 절대 주파수까지 전부 실측. 1200프레임 연속 동일값.
...
### 1.7 BTN2 니블 리드아웃 (보조 — 이제 거의 쓸 일 없음)
...
2. ~~SD 버스 검증~~ → ✅ **완료** (§1.5). `U=ff Z=0000 D=0000`
   - 남은 구멍: **픽셀 값 자체**와 **세로 2배**. `U/Z/D`는 버스 구조만 보고
     내용은 안 본다. 세로 2배는 `v_cnt_offset[0]`의 라인버퍼 경로라 버스에서
     안 보인다. 시뮬에서 `EPD_SD[15:0]`를 프레임 전체 기대 패턴과 대조할 것
