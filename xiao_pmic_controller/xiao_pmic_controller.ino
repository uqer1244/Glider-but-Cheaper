...
void loop() {
  ...
  if (current_done != last_done) {
    if (current_done == LOW) {
      Serial.println(
          "[FPGA] Screen refresh cycle STARTED (REFRESH_DONE -> LOW)");
    } else {
      Serial.println(
          "[FPGA] Screen refresh cycle COMPLETED (REFRESH_DONE -> HIGH)");
    }
...
void pmic_power_down() {
  ...
  // 1. Notify FPGA we are powering down (FPGA must stop driving EPD signals
  // immediately)
...
void print_status() {
  ...
  Serial.println(pmic_power_state ? "ON (HIGH VOLTAGES ACTIVE)"
                                  : "OFF (SLEEP)");
  ...
  Serial.println(digitalRead(PIN_FPGA_READY) ? "HIGH (Ready)"
                                             : "LOW (Not Ready)");

  Serial.print("Input  (FPGA -> XIAO) REFRESH_DONE: ");
  Serial.println(digitalRead(PIN_FPGA_DONE) ? "HIGH (EPD Idle)"
                                            : "LOW (EPD Refreshing)");
  Serial.println("--------------------------------------");
}
...
void handle_serial_cli() {
  if (Serial.available() > 0) {
    ...
    if (input.equalsIgnoreCase("help")) {
      ...
      Serial.println(
          "  status        - Print current PMIC/Handshake pin status");
      Serial.println(
          "  vcom <value>  - Set target VCOM. Options: '1.5' or '2.0'");
      ...
      if (valStr == "1.5" || valStr == "1.50" || valStr == "-1.5") {
        ...
        Serial.println(
            "Target VCOM set to -1.50V (requires power cycle/re-on to apply).");
      } else if (valStr == "2.0" || valStr == "2.00" || valStr == "-2.0") {
        current_vadj = 0x1E; // -2.00V
        Serial.println(
            "Target VCOM set to -2.00V (requires power cycle/re-on to apply).");
      } else {
        Serial.println("Invalid VCOM value! Choose '1.5' (for -1.50V) or '2.0' "
                       "(for -2.00V).");
      }
    } else {
