...
**갱신 2026-09-10.** 합성·시뮬·**P&R·실물 동작까지 통과**했다. 60.00 fps 실측,
셀프테스트 1203프레임 단일 불변식(`PROGRESS.md` §1.22).
그리고 타이밍 클로징은 "확인이 안 된" 게 아니라 **애초에 요구를 안 하고 있었다** —
`Makefile`이 nextpnr에 `--freq`를 주지 않아 기본값 12 MHz로 통과 판정하고
최적화를 멈추고 있었다. `--freq 40.5`를 추가했다.
...
- 패널 FPC를 **EE03 커넥터에 직접 꽂지 말 것**. plan.md의 **A안**(별도 브레이크아웃,
  EE03에서는 전원만 가져오기)을 쓸 것. 블로커 1(IT8951 버스 충돌)은 **아직 유효하다.**
- **갱신 2026-08-25 — EE03 전원 탭 위치 확정**(회로도 PDF 검증, plan.md §1.2/§1.5 참고):
  VGL=핀1, VGH=3, +3V3=5·11, GND=9·12·22(+34), VCOM=10, **+15V=37**, −15V=38.
  **EE03 핀 번호를 패널 핀 번호로 복사하지 말 것** — 패널 40P-A는 +15V가 36번이다(36↔37 불일치).
